...
public class PagingTableModel extends AbstractTableModel {
	...
	public int getRowCount() {
		/*
		 * int result = 0; try { Connection conn = getConnection(); Statement
		 * stmt = conn.createStatement(); ResultSet rs =
		 * stmt.executeQuery("select count(*) from Test1 where level='DEBUG'");
		 * if (rs.next()) { result = rs.getInt(1); } } catch (Exception e) {
		...
		Runnable fetch = new Runnable() {
			public void run() {
				try {
					...
					System.out.println("error retrieving page at "
							+ startOffset + ": aborting");
					pending.remove(seg);
					return;
				}
